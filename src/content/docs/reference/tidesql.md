...
That data-CF point-get is skipped entirely when the read is covering. When every column a query needs is carried by the index key - the indexed columns plus the primary-key columns appended to each entry - and each of those columns is of a type the engine can reconstruct from its comparable bytes (integers, `YEAR`, `DATE`, `DATETIME`/`TIMESTAMP`, and fixed `CHAR`/`BINARY` in a binary or latin1 charset), the row is materialised straight from the index bytes with no fetch into the data CF at all. The capability is advertised to the optimizer per key part, so a query that reads only the reconstructable columns of an index still gets an index-only plan even when the index also carries a column whose sort bytes are not invertible - `VARCHAR`, `DECIMAL`, or a floating-point column - which would otherwise be priced as a covering scan that then pays a hidden per-row primary-key lookup.

### Index Condition Pushdown (ICP)

The engine supports Index Condition Pushdown for secondary index scans. When the optimizer pushes a WHERE condition down to the storage engine, the engine evaluates it on the index key columns before performing the expensive primary key point-lookup into the data column family. This is the same pattern used by InnoDB - the engine decodes the index key columns into the record buffer and calls MariaDB's `handler_index_cond_check()` evaluator. When the condition columns are of a type the engine can decode from the index bytes - integer types (`TINYINT`, `SMALLINT`, `MEDIUMINT`, `INT`, `BIGINT`), temporal types (`DATE`, `DATETIME`, `TIMESTAMP`, `YEAR`), and fixed-length `CHAR`/`BINARY` with binary or latin1 charset - an entry that fails the condition is skipped without touching the data CF at all. For a condition column whose type cannot be reconstructed from its comparable bytes - `DECIMAL`, `VARCHAR`, floating-point, or a multi-byte-charset `CHAR` - the engine fetches the full row through the primary key first and then applies the pushed condition, so ICP still filters the result set but the data-CF fetch is not avoided for those columns.
...
Auto-increment works in a similar way to InnoDB. The engine calls MariaDB's built-in `update_auto_increment()` mechanism during `write_row()`. Rather than calling `index_last()` on every INSERT (which would create and destroy a TidesDB merge-heap iterator each time), the engine maintains an in-memory atomic counter on the shared table descriptor. The counter is seeded once at table open time by seeking to the last key in the primary key column family - but only when the AUTO_INCREMENT column is the leftmost part of the primary key, where that last key carries the highest assigned value. When the AUTO_INCREMENT column instead lives in a separate unique key, the engine leaves the counter at zero and lets MariaDB seed it on demand. It is atomically incremented via a CAS loop on each INSERT, making auto-increment assignment O(1). When a user inserts an explicit value larger than the current counter, `write_row()` bumps the counter to match.
...
MariaDB's `REPEATABLE READ` resolves through the per-table `ISOLATION_LEVEL` option. A table that leaves the option at its default (`REPEATABLE_READ`) maps to TidesDB's `SNAPSHOT` isolation, which is the semantic equivalent of InnoDB's repeatable-read, consistent read snapshot with write-write conflict detection only, no read-set tracking. TidesDB's own `REPEATABLE_READ` level is stricter (tracks read-set, detects read-write conflicts at commit) and would cause excessive conflicts under normal OLTP concurrency. A table that sets `ISOLATION_LEVEL='SNAPSHOT'`, `SERIALIZABLE`, `READ_COMMITTED` or `READ_UNCOMMITTED` is honored as written so a workload can pin a tighter or looser level on a per-table basis without changing the session default. Sessions that explicitly request `READ UNCOMMITTED`, `READ COMMITTED`, or `SERIALIZABLE` bypass the table option and use the session level.
...
Write-write conflict detection is a property of the higher isolation levels. At `SNAPSHOT` and `SERIALIZABLE` the library validates each write against the version the transaction actually read and runs a first-committer-wins reservation before the write log, so two transactions that modify the same row cannot both commit. At `READ_COMMITTED` the library performs no write-write checking at all - and every autocommit statement runs at `READ_COMMITTED` regardless of the session level, since a single-statement transaction has no concurrent modification of its own to track. At that level correctness under concurrent writes to the same row comes from the pessimistic row locks described below rather than from conflict detection, which is why the locks are on by default and why turning them off at `READ_COMMITTED` lets concurrent read-modify-write statements lose updates.

The reservation runs inside `tidesdb_txn_commit()`, so a detected conflict fails the losing transaction's `COMMIT`. The engine maps `TDB_ERR_CONFLICT` to `HA_ERR_LOCK_DEADLOCK`, but because MariaDB reports any error raised from the commit callback as `ER_ERROR_DURING_COMMIT` (ERROR 1180) rather than `ER_LOCK_DEADLOCK` (1213), a commit-time conflict surfaces to the application as 1180. The transaction is rolled back cleanly and should be retried; MariaDB does not re-run it automatically. Conflicts therefore only arise at `SNAPSHOT` or higher, and only when the pessimistic locks are off or a pattern the locks do not cover races at commit. Enabling `tidesdb_print_all_conflicts` logs every conflict event to the error log for diagnostics (see System Variables).
...
An encrypted table is the one exception. Its data column family is created with compression forced to `NONE` regardless of this option, because the rows are already ciphertext by the time they reach the library and ciphertext does not compress - running a codec over it on every flush and compaction would spend CPU for no space saving. The table's secondary-index column families hold unencrypted comparable keys and keep whatever algorithm was selected.
...
The default is 64 MB, matching `TDB_DEFAULT_WRITE_BUFFER_SIZE` in the underlying library. When `tidesdb_unified_memtable=ON` (the default) the per-CF buffer is overridden by `tidesdb_unified_memtable_write_buffer_size` since every CF shares a single memtable, so this option only affects per-CF mode.
...
`SYNC_MODE` governs how the engine fsyncs a family's SSTable files (the klog and vlog). Under `tidesdb_unified_memtable=ON` (the default) WAL durability is governed instead by `tidesdb_unified_memtable_sync_mode`, since every table shares one write-ahead log rather than a per-CF WAL; in that mode this per-table option controls only the SSTable sync for the family, not commit durability.
...
Each row is encrypted individually. The on-disk format is a 4-byte little-endian key version followed by a 16-byte random IV followed by the ciphertext. The key version prefix lets the engine decrypt rows that were written under an older version of the key after a rotation, since `encryption_key_get(key_id, key_version)` resolves the exact key bytes used at write time. On read the engine decrypts transparently. You can specify which encryption key ID to use:
...
Encryption works with all other features, including secondary indexes, BLOB columns, and TTL. The secondary index keys themselves are not encrypted (they need to remain comparable for seeking), but the row data pointed to by those keys is encrypted in the data column family. Because ciphertext is incompressible, the engine forces that data column family's compression to `NONE` regardless of the table's `COMPRESSION` option (see Compression); the index CFs are unaffected.
...
| `tidesdb_max_concurrent_flushes` | 0 | Global cap on in-flight memtable flushes. 0 (default) aligns the cap with `tidesdb_flush_threads` so every configured worker can run. Setting a value below `tidesdb_flush_threads` is honoured but leaves the surplus workers idle and logs a startup warning |
...
| `tidesdb_default_write_buffer_size` | 64 MB | Default write buffer size in bytes (matches `TDB_DEFAULT_WRITE_BUFFER_SIZE`) |
...
| `tidesdb_default_dividing_level_offset` | 1 | Default dividing level offset (matches `TDB_DEFAULT_DIVIDING_LEVEL_OFFSET`) |
