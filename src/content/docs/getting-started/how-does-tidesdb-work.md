...
Integration · When `block_cache_size` is configured, TidesDB creates two independent clock caches: one for deserialized klog blocks (block-based format) and one for deserialized B+tree nodes. The block cache uses keys formatted as "cf_name:klog_filename:block_offset" (e.g., "users:L2P3_1336.klog:65536"), while the B+tree node cache uses "sstable_id:node_offset" with hex-encoded integers. Each cache has its own eviction callback - the block cache frees reference-counted block structures, while the B+tree cache frees deserialized node structures. On cache hit, the system increments the ref_bit and returns the cached data without disk I/O or deserialization. Multiple readers share the same cached entries. The zero-copy design eliminates memory allocation on the hot read path. When an SSTable is closed or deleted, its B+tree node cache entries are invalidated by prefix to prevent use-after-free.
