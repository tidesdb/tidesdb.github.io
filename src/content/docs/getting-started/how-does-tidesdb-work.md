...
The key log uses a block-based format. Blocks target 64KB, but because each block is compressed before it is written, a block accumulates roughly twice that in uncompressed entries before it is sealed. Each block contains multiple entries serialized with variable-length integer encoding. Blocks compress independently using LZ4, LZ4-FAST, Zstd, or Snappy. The key log ends with three auxiliary structures: a block index for binary search, a bloom filter for negative lookups, and a metadata block with SSTable statistics. The on-disk layout of these components is shown in Figure 2.
<div class="architecture-diagram">
...
The block index, bloom filter, and metadata structures at the end of a key log are each written as ordinary block-manager blocks, and a single block is framed by a 4-byte size field that caps any one block at `UINT32_MAX` (just under 4GB). For most SSTables this ceiling is irrelevant: a bloom filter is bounded by its bit count `m`, which is itself capped at `UINT32_MAX`, so even a bottom-level SSTable holding hundreds of millions of keys serializes its bloom to under a gigabyte, and a block index is far smaller. Both fit in a single block.

To make the format correct for any size rather than merely large enough, an auxiliary blob that would exceed the single-block framing limit is split across multiple consecutive blocks instead of failing the write. The SSTable metadata records each chunked blob's starting offset and total size behind a flag bit (`SSTABLE_FLAG_CHUNKED_AUX`), and on open the reader reassembles the chunks into one buffer before deserializing. A blob that fits in a single block — every bloom filter and block index produced in practice — is written as exactly one block and sets no flag, so its on-disk layout is byte-identical to, and readable by, binaries built before chunking existed. The split threshold is the single-block framing limit itself, so chunking stays dormant for real data and only engages for a blob that genuinely cannot fit one block (reachable only if the bloom filter's bit count is ever widened past 32 bits). The per-column-family flush path and every compaction merge writer share one footer-writing routine, so chunking applies uniformly no matter which path produced the SSTable.

The read side never trusts a block-size field blindly. Before allocating, the block manager rejects any size whose block would extend past the file's data extent — the signature of an off-boundary read, a torn write, or corruption — and, for unusually large blocks, refuses any size beyond a memory-safety budget derived from the database memory limit, returning a graceful failure rather than attempting a multi-gigabyte allocation. A column family that cannot load an oversized bloom filter simply runs without it (falling back to block scans) instead of crashing. The [Block Manager](#block-manager) section covers this size guard in detail.
...
In per-column-family mode, write-ahead logs use the same format. Each memtable has its own WAL file, named by the SSTable ID it will become. Recovery replays these files in sequence order to reconstruct the column family's memtable state; the Recovery section describes the in-place replay of the adopted segment and asynchronous flushing of older segments in detail. In unified memtable mode, per-CF WAL files are not created since all writes go through the unified WAL. This avoids wasted I/O, file descriptors, and empty WAL file artifacts in column family directories.
...
The flush threshold is not a fixed value. At transaction commit, the system adjusts the threshold based on L0 immutable queue pressure to balance write batching against memory pressure. When the L0 queue is empty (idle), the threshold is 150% of `write_buffer_size`, providing 50% headroom and allowing the memtable to accumulate more data before flushing for better batching. When one or more immutables are pending but below half the stall threshold (moderate pressure), the threshold drops to 125% of `write_buffer_size`, providing 25% headroom. When the L0 queue depth reaches 50% or more of the effective stall threshold (`l0_queue_stall_threshold`, automatically scaled down in multi-column-family deployments), or when global memory pressure is elevated or higher, the threshold equals `write_buffer_size` exactly, with zero headroom, triggering an immediate flush. This adaptive mechanism reduces flush frequency during idle periods, improving write throughput, while ensuring rapid flushing under pressure to prevent memory buildup. With the default 64MB write buffer, the effective threshold ranges from 64MB under pressure to 96MB when idle.
...
If the system crashes after fsync but before manifest commit, the SSTable exists on disk but is not discoverable. Recovery detects it is not in the manifest and deletes it at startup. If it crashes after manifest commit but before WAL deletion, recovery finds both the committed SSTable and the leftover WAL, and replays the WAL again, producing a second SSTable with a fresh ID that holds the same data. This is harmless: every version carries its original commit sequence number, so reads still return the correct newest version and a later compaction merges the duplicate copies away.
...
With `unified_memtable_sync_mode` set to full sync, every durable commit must reach stable storage before it is made visible. If each commit issued its own `fdatasync`, throughput on slow storage would be capped by a single fsync round trip and concurrent committers would gain nothing, because each would wait on its own flush. Instead the unified WAL is opened without block-manager self-sync, and the commit path coalesces the `fdatasync` across concurrent committers. One committer becomes the leader: it issues a single `fdatasync` on the WAL and publishes how far the file is now durable, while the other committers wait for that durable offset to reach past their own write rather than each issuing a flush. A batch of N concurrent committers therefore costs one fsync instead of N. A committer that arrives with no contention takes an atomic fast path that observes the durable offset already covers its write, or issues the one fsync itself, skipping the coordinator entirely, so the low-concurrency case is unchanged.

Durable progress is tracked per WAL on the block manager, not globally, so a memtable rotation that swaps the active WAL between two batches cannot let a committer on the new WAL mistake the previous WAL's durability for its own. The fsync always completes before the commit is published as visible, so the durability guarantee is identical to per-commit syncing; only the number of syscalls changes. This coalescing applies only to the unified WAL, since per-CF mode fragments commits across separate WAL files where no single fsync can cover them all. At high concurrency in full-sync mode, unified mode sustains roughly twice the durable commits per second of per-CF mode, with parity at low concurrency.
...
When the unified memtable exceeds `unified_memtable_write_buffer_size` (which defaults to the same value as `write_buffer_size`, 64MB), it becomes immutable and is enqueued for flushing. Flushing the unified immutable runs in two phases so that per-CF SSTable writes parallelize across the worker pool rather than running sequentially in one worker. Phase one is a light scan. The dispatching worker walks the unified immutable's cursor once, and since keys are sorted by 4-byte CF index followed by the user key, consecutive entries with the same prefix belong to the same column family. The scan does not rebuild any data; it only locates each column family's contiguous prefix run and counts its entries, recording one split descriptor (CF pointer, CF index, entry count) per run. This phase is unavoidably sequential because the cursor is sequential, but it does only a single pass of cheap in-memory bookkeeping.

Phase two is the dispatch. The worker allocates a shared completion barrier and enqueues one per-CF flush task per split onto the shared flush queue, then returns. Any worker can pick up a per-CF task. Each task seeks directly into its column family's prefix run in the shared immutable skip list and streams that run straight to an SSTable in the target CF's level 1, stripping the 4-byte prefix from each key inline as it writes. There is no intermediate per-CF skip list, and no data is copied into a second structure: the entries in a run are already in sorted order, because unified mode forbids custom comparators so the shared `memcmp` order is exactly each CF's order. The task then commits the manifest, triggers a per-CF compaction if thresholds are met, and decrements the barrier. The task that brings the barrier to zero owns the unified WAL cleanup, closes the segment, optionally uploads it to the object store, and marks the unified memtable flushed. With N non-empty column families and M flush workers, the per-CF SSTable writes run roughly N over M sequentially rather than N sequentially. The on-disk format and read path through SSTables remain identical to per-CF mode.
...
The system monitors three metrics: L0 queue depth, meaning the number of immutable memtables in the flush queue (configurable threshold, default 10, aligns with the Spooky paper's stop threshold); L1 file count, meaning the number of SSTables at level 1 (configurable trigger, default 4); and active memtable size, meaning the current byte size of the active memtable (hard-capped at 2× `write_buffer_size`, see the next subsection). The system then applies increasing delays to transaction commits based on pressure, once per column family per commit.

At moderate pressure (50% of stall threshold or 3x L1 trigger), writes sleep for 0.5ms. This gently slows the write rate without significantly impacting throughput. At 50% of the default threshold (5 immutable memtables), writes experience minimal latency increase. The 0.5ms delay provides flush workers CPU time while remaining barely noticeable in multi-threaded workloads.

At high pressure (80% of stall threshold or 4x L1 trigger), writes sleep for 2ms. This more aggressively reduces write throughput to give flush and compaction workers time to catch up. At 80% of the default threshold (8 immutable memtables), write latency increases noticeably but writes continue. The 4x escalation creates a non-linear control response, giving flush and compaction workers meaningful CPU time to drain the queue.

At the stall threshold (100% or above), writes block as flow control until the queue drains below the threshold. The wait is progress-based rather than a fixed wall-clock budget. The writer keeps blocking as long as the flush engine is making headway, detected either by the L0 queue depth shrinking or by a global flush heartbeat advancing. The heartbeat is bumped by the flush writers as they iterate the memtable and by the worker as it picks up each work item, so a flush that is merely slow (a large memtable on slow disk, or many column families competing for the worker pool) keeps the writer paced rather than failing it. Only after a run of consecutive polls with zero progress, meaning neither the queue depth nor the heartbeat moved, does the stall conclude the flush engine is genuinely wedged rather than just busy and return `TDB_ERR_BUSY` (the transient "system is overloaded, retry" error, distinct from `TDB_ERR_IO` for a genuine I/O failure). The system checks every 10ms, balancing responsiveness with syscall overhead. At the default threshold of 10 immutable memtables, all writes stall until flush workers reduce the queue depth.

#### Active Memtable Ceiling

The L0 queue stall above caps the rotated immutable queue, but on its own it does not bound the active memtable that writers are still filling. With many writers, when `tidesdb_flush_memtable_internal` defers rotation because the global `active_flushes` semaphore is saturated, writers see no immediate signal to stop and the active memtable can grow far past `write_buffer_size`. To close that window, `apply_backpressure` also enforces a hard ceiling at `TDB_BACKPRESSURE_ACTIVE_MT_CEILING_MULT * write_buffer_size` (default 2x). The commit-time threshold check already allows up to 1.5x for batching headroom, the 2x ceiling leaves a small overshoot margin above that. When the active memtable's size reaches the ceiling, the writer's commit stalls with the same shape as the L0 stall: it kicks `tidesdb_flush_memtable_internal` on entry so a deferred rotate runs as soon as a slot frees, then polls in a progress-or-give-up loop using the `flush_heartbeat` counter for liveness. A run of consecutive polls with no rotation progress returns `TDB_ERR_BUSY`. The pinned read uses the `cf->active_mt_readers` epoch so a flush worker draining a just-rotated immutable cannot free the memtable struct between the size load and the wait. The per-CF branch only fires in per-CF mode (in unified mode `cf->active_memtable` stays empty), a symmetric branch on `db->unified_mt.active` runs in unified mode, kicking rotation through the same CAS plus `tidesdb_unified_memtable_rotate` sequence `tidesdb_txn_commit` uses rather than introducing a second rotator admission path.
...
Each immutable memtable holds the full contents of a flushed memtable (64MB by default). The L0 stall threshold (default 10) engages first, blocking writes once the immutable queue depth reaches that mark. Layered above the stall is a hard cap that scales with the threshold rather than being a fixed number: it is `l0_queue_stall_threshold` plus a fixed headroom of 6 (`TDB_IMM_QUEUE_HEADROOM`), which works out to 16 at the default threshold of 10. It acts as a near-absolute ceiling: under normal conditions the stall throttles writes well before the queue reaches the cap. Because the cap is derived from the configured threshold, raising `l0_queue_stall_threshold` raises the cap in lockstep instead of being silently clamped, and the lock-free immutable snapshot array grows to match so there is no hidden ceiling on how many immutables a reader can see. Per-CF memory at the stall point bounds to roughly 10 immutables times 64MB equals 640MB plus the active memtable (capped at 2x `write_buffer_size` by the active memtable ceiling, so 128MB by default), or about 770MB per column family under maximum write pressure. The two thresholds compose: the stall is the steady-state ceiling that throttles writes back to flush throughput, the hard cap is the worst-case ceiling that protects against pathological cases where the stall fails to engage (a wedged flush worker, or an open-coded flush enqueue that bypasses backpressure).

To prevent truly unbounded memory growth from immutable accumulation, the flush path enforces this hard cap (`l0_queue_stall_threshold` plus 6, so 16 at the default threshold). When the queue reaches the cap, the flush path waits for the flush worker to drain the queue below it, polling at 1ms intervals for up to five seconds, then proceeds anyway as a last resort.

#### Global Memory Pressure

Per-column-family backpressure alone cannot prevent OOM when many column families accumulate memory simultaneously. The system maintains a global memory pressure level computed by the reaper thread every 100ms. The reaper sums all active memtables, immutable memtable estimates (using `write_buffer_size` as a conservative bound on each immutable's data size), in-flight transaction memory (`txn_memory_bytes`, the staged op buffers and read-key arenas held by open transactions, accumulated per transaction and published to the global counter in batches so the per-operation path stays lock-free), compaction temporary memory estimates, bloom filter bitsets, block index arrays, and cache memory across all column families. It then computes the ratio against a resolved memory limit (configurable via `max_memory_usage`, default 50% of system RAM, minimum 5% of system RAM). Column family creation validates that the memtable arena (twice `write_buffer_size`) does not exceed the resolved memory limit. Cache sizes are validated at open time and clamped to 30% of the limit if they would exceed it.

The pressure level is graduated: normal (below 60%), elevated (60 to 75%), high (75 to 95%), and critical (95% or above). The write path reads this level with a single atomic load per commit, adding zero overhead at normal pressure. At elevated pressure, the adaptive flush threshold tightens to 100% of `write_buffer_size` (no headroom) and the write path proactively triggers a non-forced flush on the current column family if it is not already flushing, plus a 0.2ms yield to slow ingestion and prevent escalation (only the yield is skipped if L0/L1 backpressure already applied a delay on this commit; the flush still fires). At high pressure, the write path force-flushes the current column family and sleeps for 2ms (only the sleep is skipped if L0/L1 already delayed; the force-flush still fires), while the reaper force-flushes the largest non-flushing active memtable. At critical pressure, the write path performs a self-help flush on the current column family (if not already flushing) and then blocks writes entirely until the reaper brings pressure below critical, timing out after 10 seconds with `TDB_ERR_BUSY` (the transient overload signal, distinct from `TDB_ERR_IO` for a genuine I/O failure). The reaper responds to critical pressure with a nuclear flush, force-flushing every column family that is not already flushing, plus aggressive compaction on the column family with the most SSTables. In unified memtable mode the reaper sheds memory by rotating the single shared unified memtable once rather than iterating per-CF memtables. The `is_flushing` and `is_compacting` atomic flags are checked at every level to prevent redundant operations and ensure relief efforts target actionable column families.
...
The throttling mechanism distinguishes a slow flush from a stuck one. While the L0 queue is draining or the global flush heartbeat is advancing, a stalled writer waits and is simply paced to flush throughput. Only when both signals show zero progress for a sustained run of polls (roughly 10 seconds of no queue drain and no heartbeat movement) does the stall conclude the flush engine is genuinely wedged and return `TDB_ERR_BUSY`. This avoids the earlier failure mode where a fixed wall-clock budget rejected commits on a healthy but saturated system that simply could not drain L0 fast enough. A real stall of this kind typically indicates disk I/O failure, insufficient disk space, or a deadlock in the flush path. `TDB_ERR_BUSY` is the dedicated transient error code for this category, distinct from `TDB_ERR_IO` for permanent I/O failures, so a caller (or the test harness) can retry the commit instead of treating it as a hard bug.
...
The index serializes compactly using delta encoding for file positions (varints) and raw prefix bytes. The format is a 4-byte count, a 1-byte prefix length, delta-encoded file positions, min key prefixes, and max key prefixes. This achieves roughly 50% space savings compared to storing absolute positions.
...
The witness runs regardless of the structural triggers. The L1 file count and per-level capacity checks still fire on their own, but the density check is independent: a column family whose write pattern never trips a structural trigger can still cross the density threshold and force a compaction. On a witness hit the system enqueues a steer-to-bottom compaction work item for that column family; a compaction worker then merges toward the largest active level, where regular tombstones are finally allowed to drop. It is a no-op if a compaction for that column family is already running. The witness is wired into both the per-CF flush completion and the unified flush completion, so unified mode column families get tombstone relief without any extra plumbing. The witness loop itself is a small scan over a refcounted snapshot of each level, so it is cheap relative to a flush.
...
If the target level equals X, the system performs a dividing merge, consolidating all SSTables from Levels 1 through X back into level X itself. This is the default case, taken when no level shallower than X can absorb the merged data.

If the shallowest level that can absorb the combined data of Levels 1 through it falls before the dividing level X, the system performs a full preemptive merge of Levels 1 through that level into it. This handles cases where intermediate levels are filling up faster than expected.
...
It is used when the shallowest level able to hold the combined data of Levels 1 through itself lies before the dividing level X. It also serves as a fallback mechanism by the other merge functions when they cannot determine partitioning boundaries (for example, when there are no existing SSTables to use as partition guides).

The operation takes a `start_level` and `target_level` as input, opens all SSTables across that level range, and creates a min-heap containing merge sources from all of them. It iteratively pops the minimum key from the heap, writing surviving entries to new SSTables at the target level: it keeps only the newest version of each key by sequence number, drops entries whose TTL has expired, and drops a tombstone only when the target is the largest level (otherwise the tombstone is kept so it keeps shadowing older versions below). It fsyncs the new SSTables, adds them to the target level, commits the manifest, and then removes the old input SSTables.
...
The merge consolidates all SSTables from Level 1 through the dividing level X and writes the output back into level X itself. If X is the largest level in the database, the system first adds a new level beneath X (via `tidesdb_add_level`) before performing the merge, so the tree always has room to grow downward. (Dynamic Capacity Adaptation runs separately at the end of the compaction cycle to recalculate level capacities; it does not add the level itself.) The merge is intelligent about partitioning: it examines level X+1 and extracts the minimum and maximum keys from each SSTable there. These key ranges serve as partition boundaries. The key space is divided into ranges based on these boundaries, and the merge is performed in chunks. Each chunk produces an SSTable that covers only a single key range, so the consolidated output is pre-aligned with the level it will later be pushed into. This partitioning prevents the creation of monolithic SSTables and distributes data more evenly across level X. If no partitioning boundaries can be determined (for example, if level X+1 has no SSTables yet), the function falls back to calling `tidesdb_full_preemptive_merge`.
...
A compaction round is serialized per column family by the `is_compacting` flag, but the work inside a round is not single-threaded. All three merge modes split their output into independent key-range partitions and run those partitions concurrently across a bounded set of helper threads, so a single round draws on the whole compaction pool rather than occupying one worker for its entire duration. The parallelism lives inside a round; rounds themselves stay one-at-a-time per column family so level invariants and manifest ordering are preserved.

The executor is `tidesdb_run_subcompactions`. Given N partitions, it borrows up to N−1 helper threads from a database-wide `compaction_helper_budget` (initialized to `num_compaction_threads`), and the calling compaction worker participates as an additional worker. Because the budget is shared across all column families, several compaction rounds running on different column families at once can never oversubscribe the thread pool. Partitions are claimed through an atomic work-stealing cursor (`next_partition`): each worker fetch-and-increments the cursor, processes that partition, and repeats until the partitions are exhausted. Since the calling thread always works, forward progress is guaranteed even when the budget is zero or `pthread_create` fails — unclaimed partitions are simply stolen by whichever worker is free, never dropped.

Each partition owns its own merge heap and output SSTable(s) and writes them independently. Only the commit step — adding the finished output to its target level and committing the manifest — is serialized, on a per-column-family `compaction_commit_lock`, so the manifest sees one partition's commit at a time. Per-merge teardown (cleaning up the input SSTables) runs once after all partitions join.

The three modes differ only in how they derive partition boundaries. The full preemptive merge shards its single logical output into key-range subcompactions whose boundaries are sampled from the input SSTables' minimum keys (RocksDB-style subcompactions); each shard builds its heap from only the inputs overlapping its range, range-filters the merge, and writes its own output at the output level (the B+tree branch writes the shard heap unfiltered, matching the other modes). The dividing merge and partitioned merge already partition the key space using the boundary keys of the level below (or the largest level), and each of those partitions becomes a concurrent sub-compaction. When no partition boundaries can be determined, the merge degenerates to a single partition that the executor runs on the calling thread alone, identical to the earlier serial behavior.
...
DCA removes a level when several conditions are met: the largest level has become completely empty after compaction, the number of active levels exceeds the configured minimum (`min_levels`, default 1), the level was not just added in the current compaction cycle (newly added levels are intentionally empty), and no pending flushes are queued and no SSTables exist at level 1. The first two checks (`num_active_levels > min_levels` and an empty largest level) live in `tidesdb_remove_level`; the remaining guards are enforced by the caller in `tidesdb_trigger_compaction`.
...
Column families start with a minimum number of pre-allocated levels (configurable via `min_levels`, default 1). During recovery, if the manifest indicates SSTables exist at level N where N exceeds `min_levels`, the system initializes with N levels to accommodate the existing data. If SSTables exist only at lower levels, the system still initializes with at least `min_levels` levels. This floor, together with DCA, lets the tree grow and shrink with the data while keeping a predictable minimum depth. After initialization, `tidesdb_apply_dca()` is invoked to set appropriate capacities for all levels based on the actual data found during recovery.
...
Filtering and deduplication happen during this process. When multiple sources contain the same key, only the version with the highest sequence number is kept; older versions are discarded, subject to the reader-bound retention floor described under Multi-Version Concurrency Control (a superseded version whose sequence is at or above the floor is kept because a live snapshot may still need it). Entries whose time-to-live has expired are discarded. A tombstone is discarded only when the merge writes into the largest level, where no older version of the key can exist beneath the result, and only once its own sequence has fallen to or below that retention floor, so a long-running snapshot reader cannot watch a deleted key reappear mid-iteration; when merging into a shallower level the tombstone is kept so it continues to shadow older versions deeper in the tree.
...
The new SSTables are committed to the manifest file, which tracks which SSTables belong to which levels. This operation is atomic: the manifest is written to a temporary file, fsynced, and atomically renamed over the original. The merged output is added to its target level before any input is removed, so a reader always sees at least one copy of every key. The old input SSTables are then removed from the in-memory levels: each level's inputs are excised in a single atomic array swap, and the input levels are processed deepest first. Because a point read stops at the first level that holds the key, this ordering guarantees a concurrent reader sees either all of a level's merged inputs or none of them, and never observes an input's older put stranded without the tombstone that shadows it. The actual file deletion may be deferred by the reaper worker to avoid blocking the compaction worker.
...
The tombstone density witness described under Tombstone Density Trigger drives a related path: when the post-flush witness finds a sufficiently dense SSTable it enqueues a steer-to-bottom compaction work item rather than running the merge on the caller's thread. Both the synchronous `tidesdb_compact_range` API and the witness-driven compaction apply the same tombstone reclamation rules, single-delete pair cancellation, sequence-based deduplication, and value recompression, so reclamation is consistent regardless of which trigger fired.

This API captures most of the operational value of a range-tombstone primitive without the implementation cost. A range delete in TidesDB still emits per-key tombstones, but the operator can call `tidesdb_compact_range` immediately afterward to physically reclaim the affected SSTables in one pass. The trade-off versus a true range tombstone is the per-key write at delete time, and the trade-off versus waiting for natural compaction is that the caller pays the merge cost synchronously rather than amortized in the background.

The public `tidesdb_compact(cf)` call also blocks until the work item it submits has been serviced. The blocking is implemented via a per-call completion signal (mutex, condvar, flag) attached to the work item; the worker fires the signal on every exit path that consumes the item. Unlike `tidesdb_compact_range`, which runs on the caller's thread, `tidesdb_compact` enqueues onto the global compaction queue and waits for whichever worker picks the item up, so multiple `tidesdb_compact` calls from different threads can be processed in parallel across column families. The blocking call is also never coalesced against an in-flight compaction: each request runs as its own work item so the caller's intent (capture data committed up to the moment of the call) is preserved.
...
The system intelligently selects the appropriate merge strategy based on concrete triggers. When the target level equals the dividing level X, it performs a dividing merge. When the shallowest level able to absorb the cumulative data lies before X, it performs a full preemptive merge. After the initial merge, if level X remains full, it performs a partitioned merge as a secondary cleanup phase. The dividing level itself is calculated using a simple formula (`num_levels - 1 - dividing_level_offset`) rather than being inferred from complex bottleneck analysis.
...
On startup, the system reads the database-root state, scans each column family directory for write-ahead logs and SSTables, and rebuilds in-memory structures so the database resumes with the same logical state it had before the crash.

The database-root state loads first. When unified memtable mode is enabled, `tidesdb_unimap_load` reads the `UNIMAP` file before any column family is opened and populates the in-memory name-to-`unified_cf_index` map. This ordering is load-bearing: column families opened after this point pick up their persisted index, and entries replayed from the unified WAL prefix the right CF group in the shared skip list. `next_cf_index` is advanced past the maximum loaded index so newly created column families never collide with persisted ones. For each column family the manifest then loads. On a normal local restart the system recovers SSTable-to-level membership by scanning the column family directory (the level is encoded in each SSTable's filename) and uses the manifest as a completeness check, confirming each discovered SSTable finished its commit handshake. The manifest drives level membership directly only in the object-store cold-start path, where no local files exist.

`tidesdb_create_column_family` handles per-CF WAL recovery as part of the open path. It scans the column family directory for `wal_*.log`, adopts the highest-id segment, and opens it in place rather than discarding it. Validation runs in permissive mode (`block_manager_validate_last_block(bm, 0)`), which trims only the preallocation tail or a partial trailing block; the rest of the file is preserved. The system then replays every committed entry from the adopted WAL directly into the column family's active memtable. The adopted (highest-id) segment is replayed in place: the memtable a fresh open hands to the application is the memtable that WAL replayed into, and natural flush pressure later in the run promotes it to an SSTable when it crosses the threshold. Any older, lower-id WAL segments left behind by a crash are replayed into separate recovered memtables and dispatched to the flush queue asynchronously. This keeps recovery deterministic (the application observes the same memtable state regardless of how many background workers happen to be available) and removes a class of races where a recovery flush could race with the first user transaction.

The unified WAL recovers through the same mirror path. The recovery path under `tidesdb_open` adopts the highest-generation unified WAL segment, validates it permissively, and replays its entries into the unified active memtable. Because UNIMAP loaded first, each replayed entry's 4-byte CF prefix maps to the correct column family even if directory-scan order or atomic-counter state has drifted across the crash.

Sequence-counter recovery is also adjusted for the in-place model. `tidesdb_scan_max_sequence` scans the recovered SSTables (`max_seq` across every level), the immutable memtables, and the active memtable that the WAL just replayed into. The global sequence counter (`global_seq`) advances past the maximum across those sources, so a transaction that committed only to the WAL but never to an SSTable still contributes to it and remains visible to Read Committed readers in the new run. The earlier model, where replayed entries lived in a separate skip list that the visibility check did not consult, would silently filter them out.

SSTable recovery uses strict validation (`block_manager_validate_last_block(bm, 1)`): any corruption in the last block causes the SSTable to be rejected entirely. This reflects the different roles of these two kinds of files. Logs are temporary, append-only, and tolerant of preallocation tails and torn-write tails because crash recovery has to handle them. SSTables are permanent, atomically committed, and must be exactly correct because the manifest already advertised them as durable. An SSTable that does not validate did not finish its commit handshake and is removed.

## Background Workers

Background workers handle asynchronous operations (Figure 7). The flush and compaction workers are configurable thread pools; the sync worker and the reaper are single threads. Object-store mode adds an upload worker pool, and a read-only replica adds a dedicated replica sync thread.
<div class="architecture-diagram">
...
Compaction workers (configurable, default 2 threads) merge SSTables across levels. Multiple workers enable parallel compaction across column families, and within a single column family's compaction round the merge fans out across the pool via sub-compaction (see [Parallel Sub-Compaction](#parallel-sub-compaction)).

The sync worker (1 thread) periodically fsyncs write-ahead logs configured for interval sync mode. It scans all column families (and, in unified memtable mode, the unified WAL), finds the minimum sync interval, sleeps for that duration, and escalates an fsync on each interval-sync WAL. This worker is only started if a column family or the unified WAL is configured with interval sync mode during startup. If none are, it is omitted entirely.
...
The reaper worker (1 thread) runs a periodic maintenance loop, sleeping for `TDB_SSTABLE_REAPER_SLEEP_US` (100ms) between cycles. Each cycle it sweeps the deferred-free list (retired SSTable arrays), retries any memtable flushes that were deferred under the concurrency cap, drains any column families whose `compaction_armed` flag was set by an enqueue that arrived while a compaction was already running (so a trigger that the worker missed is still serviced), recomputes global memory pressure and acts on it, and evicts unused SSTable file handles when the open count exceeds the limit. In object-store mode it also enqueues the periodic unified WAL sync onto the upload worker pool. Replica MANIFEST sync and WAL replay are handled by a separate dedicated thread, not the reaper.

### Global Memory Pressure

Each cycle, the reaper scans all column families to compute total memory usage: active memtable sizes (via atomic load), immutable queue estimates, bloom filter bitsets, block index arrays, and cache bytes. It divides this total by the resolved memory limit to produce a pressure level (normal, elevated, high, critical) stored atomically for the write path to consume. The reaper uses the `is_flushing` and `is_compacting` atomic flags to avoid redundant operations and target only actionable column families. When selecting a flush victim, it picks the column family with the largest active memtable that is not already flushing (mirroring the compaction victim selection which already filters by `!is_compacting`). At high pressure, the reaper force-flushes this single largest non-flushing column family. At critical pressure, the reaper performs a nuclear flush, force-flushing every column family that is not already flushing, to shed memory as fast as possible across the entire database. In unified memtable mode, where all writes share a single memtable, the reaper instead rotates that unified memtable once under its `is_flushing` admission gate, since force-flushing the empty per-CF memtables would shed nothing. In both cases, the reaper also triggers aggressive compaction on the column family with the most SSTables (that is not already compacting), merging N SSTables into 1 to free N-1 bloom filters and block indexes, producing tighter replacements sized to the exact merged entry count.
...
When the open SSTable count reaches the open budget (`tidesdb_sstable_open_budget`, which is `max_open_sstables` minus the reader fd reserve held for the write path, rather than `max_open_sstables` itself), the reaper sorts open SSTables by last access time (updated atomically on each SSTable open, not on every read) and closes the oldest `TDB_SSTABLE_REAPER_EVICT_RATIO` (25%). `max_open_sstables` defaults to 256 SSTables, equivalent to 512 file descriptors; the default is 64 on OpenBSD, which has lower file-descriptor limits. Evicting down to the budget keeps the reserve free for flush and compaction; with persistently more SSTables than the budget, the reaper runs the eviction logic continuously, causing file descriptor thrashing.
...
Workers call `queue_dequeue_wait()` to block until work arrives. Multiple workers can process different column families simultaneously, and worker 1 might flush column family A while worker 2 flushes column family B. Each column family uses atomic flags (`is_flushing`, `is_compacting`) with compare-and-swap to coordinate operations. The `is_flushing` flag is now a brief rotate-critical-section gate, held only during the active-memtable swap and the work enqueue, then cleared by the writer thread before it returns. Concurrency on the actual SSTable writes is bounded instead by a database-wide counter `active_flushes` capped at `max_concurrent_flushes`. At open time `max_concurrent_flushes` is pinned 1:1 to `num_flush_threads` (a higher cap would be meaningless because the pool size is the hard upper bound, a lower cap would leave workers idle); any caller-supplied mismatch is normalized to `num_flush_threads` and emits a `TDB_LOG_WARN`. This counter behaves like a semaphore. A hot column family can have multiple memtables flushing concurrently up to the cap, so a single CF drains its immutable queue at the speed of the worker pool rather than serially. The same cap also bounds total transient memory and queue depth in deployments with many column families, since the cap is global rather than per-CF. The writer increments the counter before enqueueing the flush work item and the worker decrements it after the SSTable commits, so a stuck or slow worker never lets the in-flight count drift. Compaction rounds remain one-per-CF via `is_compacting` to preserve level invariants, though the work inside a round fans out across the pool through sub-compaction (see [Parallel Sub-Compaction](#parallel-sub-compaction)). Triggers that arrive while `is_compacting` is held set a per-CF `compaction_armed` flag rather than being dropped silently; the worker exchanges the flag back to zero after each compaction finishes and self-enqueues a follow-up if it was set, and the reaper drains the flag as a backstop. The public `tidesdb_is_flushing()` and `tidesdb_is_compacting()` functions check the atomic flag and the respective per-CF pending-work counter (`flush_pending_count` / `compaction_pending_count`), returning true if either indicates pending work.

Because `is_flushing` is now only a rotate gate rather than a "work in progress" signal, the right indicator for "is anything actually being written" is the per-CF `flush_pending_count`. The flush-completion wait polls that counter rather than `is_flushing` so a caller that asked for the active memtable to land on disk does not return the moment the rotate critical section clears, before the SSTable write has even started. The same applies to the internal helpers that the unified flush dispatcher and the purge paths use to coordinate barrier completion. Operators reading these states externally should treat `is_flushing` as "a rotation is in flight" and `flush_pending_count` as "queued or running flushes exist."
...
The parallelism semantics are straightforward. Multiple flush and compaction workers can process different column families in parallel (cross-CF parallelism). For flushes, a single column family can also have multiple memtables flushing concurrently up to the global `max_concurrent_flushes` cap, so a hot CF drains its immutable queue at the speed of the worker pool rather than serially. Compaction rounds stay serialized per CF (via `is_compacting`) so level invariants and manifest ordering stay consistent, but the work inside a round fans out across the compaction pool through sub-compaction (see [Parallel Sub-Compaction](#parallel-sub-compaction)), bounded by the shared `compaction_helper_budget`. The global flush cap balances per-CF throughput against total transient memory in deployments with many column families.

Thread pool sizing follows from these semantics. For a single column family with sustained write pressure, the flush thread count effectively bounds how fast the immutable queue drains because multiple flushes per CF are now allowed. For multiple column families, set the flush thread count up to roughly the number of concurrently hot CFs (`max_concurrent_flushes` will follow it via the 1:1 pin) and compaction thread counts up to the number of column families. With N column families and M flush workers, flush latency on quiet CFs is roughly N over M times the flush time. The global queue provides natural load balancing. For mixed workloads where some CFs are write-heavy and others read-heavy, the thread pool automatically prioritizes work from active CFs.
...
If the pre-flush disk-space check fails, the flush is skipped: the work item is freed and its immutable memtable is released, and the memtable stays in memory. If an SSTable write fails partway through (for example, the disk fills mid-flush), the partial SSTable is discarded and the work item is re-enqueued for retry after a short delay, keeping the immutable reference held; the item is only dropped if the re-enqueue itself fails. In all cases writes continue to the active memtable. When the active memtable fills and backpressure cannot drain the queue, writes stall. The system logs the failure but does not fail individual writes until memory is exhausted.
...
In leveled compaction, each entry gets rewritten once per level it passes through. With ratio R and L levels, average write amplification is approximately R * L / 2 (not R * L) because data at shallow levels gets rewritten more than data at deep levels. For a 1TB database with default 64MB L1 and ratio 10, the tree settles at about 6 levels (64MB grown 10x per level reaches roughly 6.4TB at level 6, the first level whose capacity exceeds 1TB), so average write amplification is roughly 30x rather than the 60x a naive R * L estimate would suggest. Actual write amplification depends on workload; updates to existing keys have lower write amplification than pure inserts.

### Read Amplification

Worst case reads one SSTable per level. With 6 levels, that is 6 disk reads without bloom filters. Bloom filters (1% FPR) reduce this. Expected reads are approximately 1 + 6 * 0.01 = 1.06 for absent keys. This is an approximation valid for small FPR (probability of no false positives across all levels is roughly 0.99^6, or about 0.94). For present keys, bloom filters do not help; the system still needs to read the actual block.
...
The `max_memory_usage` configuration (default 0 for auto) sets an upper bound on total tracked memory across all column families. When set to 0, the system resolves it to 50% of total system RAM at startup. A user-supplied non-zero value is honored but clamped up to a floor of 5% of total RAM, so the limit can never be set pathologically low. The reaper thread monitors the aggregate of memtables, caches, bloom filters, and block indexes against this limit, applying graduated pressure to the write path and triggering force-flushes and aggressive compaction when usage exceeds thresholds. This prevents OOM in multi-column-family deployments where per-CF limits alone cannot bound aggregate memory consumption.

### Compaction Lag

Writes can outpace compaction if the write rate exceeds the compaction throughput. The system applies backpressure: when L0 exceeds 10 immutable memtables (configurable via `l0_queue_stall_threshold`), writes stall until flush workers catch up. This prevents unbounded memory growth but can cause write latency spikes.
...
An operator who wants a larger working set of open SSTables can call `tidesdb_raise_open_file_limit(desired)` before `tidesdb_open()` to raise the process's open-file ceiling toward `desired` (on POSIX this raises the `RLIMIT_NOFILE` soft limit toward the hard limit, on Windows it raises the CRT stdio cap). The engine then sizes `max_open_sstables` to fit the larger ceiling at open time. The call is opt-in; TidesDB never raises the limit on its own, and a failed or partial raise is non-fatal, leaving the prior ceiling in place. Passing a value at or below the current ceiling just reports it.
...
Block reads use two `pread` syscalls: one for the 8-byte header (size plus checksum) and one for the data payload into a reusable per-thread read buffer. The payload is checksum-verified in that buffer, and only then is the block's final allocation made and the bytes copied in, so a block that fails verification never costs a heap allocation. The fused `block_manager_cursor_read_and_advance()` operation combines read and cursor advance into a single call, using the block size from the just-read block to compute the next position without a redundant `pread`. Cursors also cache block sizes from previous operations, allowing `cursor_read_partial()` to skip the size lookup when the cache is valid. These optimizations reduce syscall overhead on the hot read path.

The cached size has a precise contract. `current_block_size` is the on-disk (compressed, framed) size of the block at `current_pos`, and `cursor_next` advances `current_pos` by `header + current_block_size + footer`. Callers that work with decompressed payloads must not write the decompressed size back into the cursor cache; they either set `current_block_size` from the just-read on-disk block (`bmblock->size` when the read came from disk) or invalidate `block_size_valid` so `cursor_next` re-reads the size header on its next call. The cost of the invalidation is one extra `pread` of four bytes, which is negligible compared to the disaster of advancing into the middle of a block on a workload with compression enabled.

`block_manager_read_block_at_offset` defends against a stale or wrong offset, corruption, and pathological sizes without ever trusting the 4-byte block-size field blindly. Before any allocation it computes the block's full extent (header + payload + footer) and rejects a size whose block would run past the file's data extent -- the signature of an off-boundary read, a torn write, or corruption -- returning NULL with a structured diagnostic (offset, claimed extent, file size, raw header bytes) instead of allocating from garbage. For unusually large blocks (above a fixed threshold that ordinary data blocks and small footer blocks never reach) it additionally refuses any size beyond a memory-safety budget pushed down from the database's resolved memory limit; that check is a single relaxed atomic load, so the hot read path makes no memory syscall and small blocks pay nothing. This converts the rare crash mode of "a cursor landed at a non-boundary offset and asked the allocator for the raw bytes there" into a graceful read failure on every platform, including 32-bit hosts where the spurious allocation would otherwise OOM the process. An earlier version used a fixed 256 MB ceiling, but that wrongly rejected legitimately large bloom-filter footer blocks on very large bottom-level SSTables (silently disabling their bloom filters on reopen); the extent-relative and budget-relative checks accept any valid block that fits both the file and the memory budget while still catching genuinely absurd sizes.
...
The filter serializes using varint encoding for headers and sparse encoding for the bitset, storing only non-zero words with their indices. This achieves 70 to 90% space savings for low fill rates (below 50%). The serialization format is an optional version marker (a 0x00 sentinel byte followed by a hash-version byte, written only by version 2 and newer filters and unambiguous because a legacy filter always begins with varint(m) where m is at least 1), then varint(m), varint(h), varint(non_zero_count), then pairs of varint(index) and varint(word).

The hash function uses Kirsch-Mitzenmacher double hashing. Two base hashes (h1 and h2) are computed using a multiplicative hash with seeds 0 and 1, then finalized with a murmur3 fmix32 step so that even a short key fully avalanches and the two derived hashes stay independent. The i-th hash function is derived as `h1 + i * h2`, producing h independent bit positions from only two hash computations. Each hash sets one bit in the bitset using Lemire's fast range reduction for uniform distribution across the bitset.

The index hash is versioned. A filter records which hash built it and is always queried with that same hash, so SSTables written before the fmix32 finalizer was introduced keep querying with the original hash and never produce false negatives, while newly written SSTables get the lower false positive rate on short keys. The improvement spreads naturally as compaction rewrites old files into new ones.
...
When `block_cache_size` is configured, TidesDB creates two independent clock caches: one for raw klog block bytes (block-based format) and one for deserialized B+tree nodes. The block cache stores raw bytes (decompressed but not deserialized) inline, using keys formatted as "cf_name:klog_filename:block_offset" (for example, "users:L2P3_1336.klog:65536"). This yields roughly 20x smaller cache entries compared to caching deserialized blocks, dramatically improving cache hit rates (87% versus 3.7% for a 64MB cache with an 80MB dataset). The B+tree node cache keys each node by a hex-encoded 64-bit hash of the SSTable's key-log path combined with the node offset, and stores fully deserialized node structures. On a block cache hit, the cached bytes (already decompressed) are pinned zero-copy and the caller deserializes directly from them. The `clock_cache_put_new` fast path skips redundant hash probes on cache-miss insertions since the caller just confirmed the key was absent. On B+tree cache hit, the system increments the ref_bit and returns the cached node without disk I/O. When an SSTable is closed or deleted, its B+tree node cache entries are invalidated by prefix to prevent use-after-free.
...
TidesDB uses one manifest per column family. During flush, the system adds the new SSTable to the manifest and fsyncs before deleting the WAL. During compaction, it adds new SSTables and removes old ones atomically. During recovery, the manifest is the authority for which SSTables are durably committed: a local restart cross-checks the directory scan against it and deletes any SSTable absent from the manifest as an incomplete write, while object-store cold start reconstructs level membership directly from it. The manifest is the source of truth for which SSTables are durably part of the LSM tree.

### Platform Compatibility (compat.h)

The `compat.h` header isolates all platform-specific code, enabling TidesDB to run on Windows (MSVC, MinGW), macOS, Linux, BSD variants, and Solaris/Illumos without changes to the core implementation. I/O operations (`pread`/`pwrite`, `fdatasync`) map to Windows equivalents (`ReadFile`/`WriteFile` with `OVERLAPPED`, `FlushFileBuffers`). Atomics use C11 `stdatomic.h` on modern compilers or Windows `Interlocked*` functions on older MSVC. Threading uses POSIX `pthread` (pthreads-win32 on MSVC, native on MinGW). File system operations (`opendir`/`readdir`) map to Windows `FindFirstFile`/`FindNextFile`. Semaphores use Windows APIs on MSVC, native `semaphore.h` elsewhere. Type definitions handle platform differences (`off_t`, `ssize_t`, format specifiers). Performance hints (`PREFETCH_READ`, `TDB_LIKELY`, `TDB_UNLIKELY`) use compiler intrinsics where available. Every source file includes `compat.h` first. The abstraction layer has zero runtime overhead: all macros and inline functions compile to native platform calls.
...
SSTable files are stored across two physical tiers (Figure 8). The local tier is the database directory on local disk. All writes, flushes, and compactions produce files here using the same format and machinery as non-object-store mode. The remote tier is the object store. Files are uploaded after creation and downloaded on demand when not present locally. Layered over these two storage locations is a finer access hierarchy, described below, that classifies data by how fast a read of it will be.
<div class="architecture-diagram">
...
Across those two storage tiers, the data a read might touch falls into one of four access states — a hot, warm, cold, and frozen hierarchy that separates data by access recency.
...
The write path is unchanged at the memtable and WAL level. All writes go to the unified memtable and unified WAL as usual. When the memtable is flushed, the flush worker writes per-CF SSTables to local disk. After `tidesdb_level_add_sstable` writes the SSTable locally, it uploads the klog and vlog files synchronously to the object store with retry (3 attempts with exponential backoff). Post-upload size verification is performed on the asynchronous upload-worker path, not on this synchronous upload. The synchronous upload ensures the object store has a copy before local cache eviction can delete the file. The MANIFEST is uploaded asynchronously after each flush and compaction when `sync_manifest_to_object` is enabled (default), ensuring the remote store has the latest SSTable inventory without blocking the flush worker. The `UNIMAP` file is uploaded alongside `config.ini` and `MANIFEST` whenever the in-memory map is rewritten (column family create, drop, or rename), so a cold-started replica or freshly promoted node reconstructs the same `unified_cf_index` values the primary assigned rather than picking its own. Closed unified WAL segments are uploaded to the object store before local deletion when `replicate_wal` is enabled, with upload timing controlled by `wal_upload_sync` (synchronous or background via the upload thread pool).

### Multipart Upload

The S3 connector splits large uploads so it never buffers a whole SSTable in memory. A file below the 64MB multipart threshold is sent as a single streaming `PUT`. A file at or above it is uploaded as an S3 multipart upload: the connector issues `CreateMultipartUpload`, streams the file in fixed 8MB parts (`PUT` per part, collecting each part's ETag), and finishes with `CompleteMultipartUpload` carrying the XML manifest of part numbers and ETags. Any failure along the way triggers `AbortMultipartUpload` so no orphaned parts linger. Because parts are read and sent one at a time, connector memory stays bounded to a single 8MB part buffer regardless of file size, and the upload is not constrained by S3's 5 GiB single-`PUT` limit (S3 requires parts of at least 5 MiB and at most 10000 parts). The 64MB threshold (`TDB_S3_MULTIPART_THRESHOLD`) and 8MB part size (`TDB_S3_MULTIPART_PART_SIZE`) are the connector defaults; the `multipart_threshold` and `multipart_part_size` fields on the S3 connector config (`tidesdb_objstore_s3_config_t`) override them when non-zero, falling back to these defaults otherwise. The multipart fields on the database-level `tidesdb_objstore_config_t` are not plumbed through to the connector. The filesystem connector writes objects as plain local files and does not use multipart.
...
When the object store is unavailable or range_get returns an error, reads fall through to `tdb_objstore_download_if_missing`, which downloads the full file with retry (3 attempts, exponential backoff 50ms/200ms/800ms). The download checks whether the object exists before attempting retrieval. Only a confirmed hit (`exists` returns 1) triggers a download. A definitive miss or a head error both skip the download and let `block_manager_open` create the local file. This matters most for fresh SSTables produced by flush, whose objects do not exist remotely yet, and is harmless for read paths because a transient head failure that hides a real remote object is corrected on the next access. Downloaded files are tracked by the local cache and subject to LRU eviction.
...
Before each MANIFEST sync cycle, the replica sync thread re-downloads the remote `UNIMAP` so any column family the primary added, dropped, or renamed is reflected in the replica's in-memory map before new CFs are opened. It then discovers new column families in the object store that do not exist locally and creates them automatically by downloading their config and MANIFEST. With UNIMAP synced first, the replica's newly opened column families pick up the primary's `unified_cf_index` values, which is the only way the prefixes in WAL replay can resolve to the right CF groups. The replica sync thread then polls the remote MANIFEST for each column family every `replica_sync_interval_us` (default 5 seconds). `tdb_replica_sync_manifests` downloads each CF's MANIFEST to a temporary file, diffs it against the local MANIFEST, adds new SSTables (following the same pattern as cold start recovery), and removes SSTables that the primary compacted away. SSTable data is not downloaded during sync; it is fetched on demand via range_get for point lookups or prefetch for iterators.

When `replica_replay_wal` is enabled (default), the replica sync thread discovers all available unified WAL segments from the object store via listing, sorts them by generation, and replays new entries from each into the unified memtable. Replay is idempotent using sequence numbers. Entries with sequence numbers strictly below the current maximum are skipped; equal-sequence entries (sibling puts from the same commit, which all share one commit sequence) are re-applied, and the skip list rejects any duplicate `(key, sequence)` pair, so re-replay is harmless. This gives the replica access to writes that the primary committed but has not yet flushed to SSTables, even when the primary has rotated through multiple WAL generations between sync intervals. The replica's memtable is ephemeral and rebuilt from the primary's WAL segments on each sync cycle.
...
When the primary fails, a replica can be promoted to primary via `tidesdb_promote_to_primary`. The function first stops and joins the dedicated replica sync thread, draining any in-progress sync to avoid contention with the first post-promotion query, then performs a final MANIFEST sync and WAL replay to capture any last writes from the old primary, creates a local WAL for crash recovery of new writes, and atomically switches `replica_mode` to 0. After promotion, the node accepts writes immediately.
...
Upload failures are tracked via `total_upload_failures` on `tidesdb_db_stats_t`, allowing operators to monitor object store health. The shutdown log reports both successful uploads and permanent failures. Download failures after all retry attempts return `TDB_ERR_IO` to the caller, which propagates to the transaction as a read error. The `exists` check before downloads only triggers a download on a confirmed hit. A definitive miss or a head error both skip the download, so flush paths that just created an SSTable locally proceed without spurious retries against an object that does not exist remotely yet. At shutdown, the upload pipeline sends poison pill sentinels to each worker thread and calls `queue_shutdown` to broadcast-wake all blocked workers, ensuring prompt thread termination on all platforms including MinGW where condition variable timeouts may not behave reliably.

## Testing and Quality Assurance

TidesDB employs comprehensive testing with CI/CD automation across more than 15 platform and architecture combinations. Each internal component has dedicated test files (`block_manager__tests.c`, `skip_list__tests.c`, `bloom_filter__tests.c`, and others) with unit tests, integration tests, and performance benchmarks. The main integration suite (`tidesdb__tests.c`) contains tests covering the full database lifecycle: basic operations, transactions across all isolation levels, persistence, WAL recovery, compaction strategies, iterators, TTL, compression, bloom filters, block indexes, concurrent operations, edge cases, and stress tests. This is not including the tests within each modular component. Test utilities (`test_utils.h`) provide assertion macros, execution harnesses with colored output, and test name filtering. Any test binary accepts an optional command-line argument to run only tests whose names contain the given substring (for example, `./build/tidesdb_tests checkpoint` runs only checkpoint-related tests).

The CMake build system automatically adapts to Linux, macOS, Windows (MSVC and MinGW via MSYS2), BSD variants, and Solaris/Illumos, detecting the platform and, on macOS, Intel versus Apple Silicon. The specific architecture matrix (x64, x86, PowerPC, RISC-V) is exercised by the CI workflow, which supplies the cross-compilation toolchains. CMake manages dependencies via vcpkg (Windows with binary caching), Homebrew (macOS), and pkg-config (Linux), handles cross-compilation for PowerPC with custom-built dependencies, enables sanitizers (AddressSanitizer, UndefinedBehaviorSanitizer) on Unix platforms, provides 30 or more benchmark configuration variables, and registers tests with CTest for execution. The installed shared library carries a soname (`libtidesdb.so.MAJOR`) and the install paths follow OS conventions (`lib/` and `include/` on Unix-like systems, the standard locations on Windows), so packagers and downstream linkers pick up the right files without per-distro patches.

GitHub Actions CI builds and tests more than 15 platform and architecture combinations, installs compression libraries (zstd, lz4, snappy) and pthreads on each platform, cross-compiles PowerPC builds with dependencies built from source, and runs tests via CTest on native platforms and under QEMU emulation for RISC-V. PowerPC builds are verified but their runtime tests are skipped, as QEMU user-mode emulation of PowerPC is unreliable for pthreads and atomics. MinGW jobs run under MSYS2 with a single code path for both x64 and x86: snappy is built from source (version 1.2.2) for both architectures because MSYS2 dropped i686 prebuilts in October 2024, and `find_package(Snappy CONFIG REQUIRED)` resolves the result on both. zstd and lz4 use `pkg_check_modules` on MinGW since MSYS2 ships only `.pc` files for them. A cross-platform portability test creates a database on Linux x64, uploads it as an artifact, downloads it on 7 different platforms, and verifies all keys are readable with correct values, proving the database format is truly portable across architectures and endianness. Windows builds use vcpkg binary caching to substantially reduce build times on cache hits. When it comes to object storage there is dedicated workflows testing object store capabilities utilizing local S3 supported storage and by default we use MinIO though this will be replaced soon.  
