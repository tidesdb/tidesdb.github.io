...
Each immutable memtable holds the full contents of a flushed memtable (64MB by default). The L0 stall threshold (default 10) engages first, blocking writes once the immutable queue depth reaches that mark. Layered above the stall is a hard cap of 16 immutable memtables (`TDB_IMMUTABLE_HARD_CAP`) per column family that acts as a near-absolute ceiling: under normal conditions the stall throttles writes well before the queue ever reaches 16. Per-CF memory at the stall point bounds to roughly 10 immutables times 64MB equals 640MB plus the active memtable (capped at 2x `write_buffer_size` by the active memtable ceiling, so 128MB by default), or about 770MB per column family under maximum write pressure. The two thresholds compose: the stall is the steady-state ceiling that throttles writes back to flush throughput, the hard cap is the worst-case ceiling that protects against pathological cases where the stall fails to engage (a wedged flush worker, or an open-coded flush enqueue that bypasses backpressure).

To prevent truly unbounded memory growth from immutable accumulation, the flush path enforces the hard cap of 16 immutable memtables per column family. When the queue reaches this limit, the flush path waits for the flush worker to drain the queue below the cap, polling at 1ms intervals for up to five seconds, then proceeds anyway as a last resort.
...
Filtering and deduplication happen during this process. When multiple sources contain the same key, only the version with the highest sequence number is kept; older versions are discarded. Entries whose time-to-live has expired are discarded. A tombstone is discarded only when the merge writes into the largest level, where no older version of the key can exist beneath the result; when merging into a shallower level the tombstone is kept so it continues to shadow older versions deeper in the tree.
...
TidesDB allocates a per-column-family `active_txn_buffer` from this component (configurable, default 64K slots, wired with a `txn_entry_evict` eviction callback). It is reserved for active-transaction tracking; in the current implementation, serializable-isolation conflict detection scans the database-wide active-transaction list directly, so the per-CF buffer is allocated but not yet exercised on the conflict-detection path.
