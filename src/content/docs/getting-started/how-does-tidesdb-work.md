...
`block_manager_read_block_at_offset` defends against a stale or wrong offset with a size sanity gate. The 4-byte block-size field at the read offset is checked against `BM_DIAG_MAX_SANE_BLOCK_SIZE` (256 MB) before any allocation happens; a value above the bound logs a structured diagnostic (offset, file path, file size, raw header bytes) and returns NULL rather than asking `realloc` for a multi-gigabyte buffer. The guard is silent on the hot path because well-formed blocks always pass the check, but it converts the rare crash mode of "iterator landed at a non-boundary offset and asked the allocator for the raw bytes there" into a graceful read failure on every platform, including 32-bit hosts where the spurious allocation would otherwise OOM the process.
