...
Because `is_flushing` is now only a rotate gate rather than a "work in progress" signal, the right indicator for "is anything actually being written" is the per-CF `flush_pending_count`. `wait_for_flush_complete` polls that counter rather than `is_flushing` so a caller that asked for the active memtable to land on disk does not return the moment the rotate critical section clears, before the SSTable write has even started. The same applies to the internal helpers that the unified flush dispatcher and the purge paths use to coordinate barrier completion. Operators reading these states externally should treat `is_flushing` as "a rotation is in flight" and `flush_pending_count` as "queued or running flushes exist."
