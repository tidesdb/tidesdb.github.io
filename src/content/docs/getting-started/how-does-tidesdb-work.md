...
The parallelism semantics are straightforward. Multiple flush and compaction workers can process different column families in parallel (cross-CF parallelism). For flushes, a single column family can also have multiple memtables flushing concurrently up to the global `max_concurrent_flushes` cap, so a hot CF drains its immutable queue at the speed of the worker pool rather than serially. Compaction remains within-CF serialized so level invariants and manifest ordering stay consistent. The global flush cap balances per-CF throughput against total transient memory in deployments with many column families.
...
If the disk fills during flush, the flush fails; the flush work item is dropped and its immutable memtable is released. Writes continue to the active memtable. When the active memtable fills and backpressure cannot drain the queue, writes stall. The system logs the failure but does not fail individual writes until memory is exhausted.
...
The S3 connector splits large uploads so it never buffers a whole SSTable in memory. A file below the 64MB multipart threshold is sent as a single streaming `PUT`. A file at or above it is uploaded as an S3 multipart upload: the connector issues `CreateMultipartUpload`, streams the file in fixed 16MB parts (`PUT` per part, collecting each part's ETag), and finishes with `CompleteMultipartUpload` carrying the XML manifest of part numbers and ETags. Any failure along the way triggers `AbortMultipartUpload` so no orphaned parts linger. Because parts are read and sent one at a time, connector memory stays bounded to a single 16MB part buffer regardless of file size, and the upload is not constrained by S3's 5 GiB single-`PUT` limit (S3 requires parts of at least 5 MiB and at most 10000 parts). The 64MB threshold and 16MB part size are fixed connector constants; the `multipart_threshold` and `multipart_part_size` fields on `tidesdb_objstore_config_t` are reserved and not currently wired to them. The filesystem connector writes objects as plain local files and does not use multipart.
...
GitHub Actions CI builds and tests more than 15 platform and architecture combinations, installs compression libraries (zstd, lz4, snappy) and pthreads on each platform, cross-compiles PowerPC builds with dependencies built from source, and runs tests via CTest on native platforms and under QEMU emulation for RISC-V. PowerPC builds are verified but their runtime tests are skipped, as QEMU user-mode emulation of PowerPC is unreliable for pthreads and atomics. MinGW jobs run under MSYS2 with a single code path for both x64 and x86: x64 picks up snappy from MSYS2's prebuilt packages, x86 builds snappy from source into `/mingw32` because MSYS2 dropped i686 prebuilts in October 2024, and `find_package(Snappy CONFIG REQUIRED)` resolves both. zstd and lz4 use `pkg_check_modules` on MinGW since MSYS2 ships only `.pc` files for them. A cross-platform portability test creates a database on Linux x64, uploads it as an artifact, downloads it on 7 different platforms, and verifies all keys are readable with correct values, proving the database format is truly portable across architectures and endianness. Windows builds use vcpkg binary caching to reduce build times from over 20 minutes to 2 to 3 minutes on cache hits. When it comes to object storage there is dedicated workflows testing object store capabilities utilizing local S3 supported storage and by default we use MinIO though this will be replaced soon.  
