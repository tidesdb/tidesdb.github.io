...
To prevent truly unbounded memory growth from immutable accumulation, the flush path enforces a hard cap of 16 immutable memtables per column family. When the queue reaches this limit, the flush path waits for the flush worker to drain the queue below the cap, polling at 1ms intervals for up to five seconds, then proceeds anyway as a last resort. This complements the L0 stall threshold (which slows writes) by providing a near-absolute ceiling on immutable memory.
...
Workers call `queue_dequeue_wait()` to block until work arrives. Multiple workers can process different column families simultaneously, and worker 1 might flush column family A while worker 2 flushes column family B. Each column family uses atomic flags (`is_flushing`, `is_compacting`) with compare-and-swap to coordinate operations. The `is_flushing` flag is now a brief rotate-critical-section gate, held only during the active-memtable swap and the work enqueue, then cleared by the writer thread before it returns. Concurrency on the actual SSTable writes is bounded instead by a database-wide counter `active_flushes` capped at `max_concurrent_flushes` (default 4). This counter behaves like a semaphore. A hot column family can have multiple memtables flushing concurrently up to the cap, so a single CF drains its immutable queue at the speed of the worker pool rather than serially. The same cap also bounds total transient memory and queue depth in deployments with many column families, since the cap is global rather than per-CF. The writer increments the counter before enqueueing the flush work item and the worker decrements it after the SSTable commits, so a stuck or slow worker never lets the in-flight count drift. Compaction remains one-per-CF via `is_compacting` to preserve level invariants. The public `tidesdb_is_flushing()` and `tidesdb_is_compacting()` functions check the atomic flag and the respective work queue size, returning true if either indicates pending work.
