...
I am using <a href="https://hammerdb.com">HammerDB</a> for this work utilizing TPROC-C.   
...
The command below runs one TPC-C iteration against the TidesDB engine on a MariaDB reachable at /tmp/mariadb.sock as hammerdb / hammerdb123, using HammerDB 5.0 under ~/HammerDB-5.0. It builds a 40-warehouse TPC-C schema with 8 build VUs, settles for 5 s, runs the timed workload with 8 measured VUs for 1 minute of ramp-up + 2 minutes of measurement, also capturing the NOPM/TPM/p95 numbers.
```bash
...
Thus the command used in the article was below and it does one TPC-C iteration each for TidesDB >> RocksDB >> InnoDB.
...
I use a specific small buffer/cache setup that is optimized and aligned for TidesDB, RocksDB and InnoDB you can find that here: <a href="https://github.com/tidesdb/hammer/blob/master/my.cnf.small.example">https://github.com/tidesdb/hammer/blob/master/my.cnf.small.example</a>
...
On consumer hardware with an aligned 1 GB block cache per engine, durability disabled, READ COMMITTED isolation, and 40 warehouses against 8 measured VUs, both LSM engines outperform InnoDB by 5x to 6x on NOPM. TidesDB leads MyRocks by roughly 25 percent on every aggregate metric and beats it on the p95 of every transaction class. 
