...
Examples:
  ...
  # All engines including PostgreSQL
  ./tidesdb_rocksdb_hammerdb.sh -e all --pg-superuser-pass mypass
...
find_postgres_pid() {
    ...
    # Return the postmaster (parent) PID
    echo "$pid" | head -1
}

# Helper: is this engine PostgreSQL?
is_pg_engine() {
    [[ "$1" == "PostgreSQL" ]]
}
...
EC = {"TidesDB": "#6FA8DC", "RocksDB": "#B0B0B0", "InnoDB": "#F4B183", "PostgreSQL": "#8BC34A"}
EE = {"TidesDB": "#4A7FB5", "RocksDB": "#808080", "InnoDB": "#C47D3E", "PostgreSQL": "#558B2F"}
...
if tpch_rows:
    ...
    fig.suptitle("TPC-H: Query Performance Comparison", fontsize=13, fontweight="bold", y=1.02)
    fig.tight_layout()
